...
- [ ] `app.py` 파일 생성
- [ ] Streamlit 페이지 설정 (title, icon, layout)
- [ ] 기본 레이아웃 구성 (헤더, 메인, 푸터)
- [ ] 페이지 스타일 CSS 작성
- [ ] 가상환경에서 앱 실행 테스트
  ```bash
...
- [ ] 탭 위젯 구현 (JMeter→LoadRunner / LoadRunner→JMeter)
- [ ] 탭 전환 이벤트 핸들러
- [ ] 선택된 방향에 따른 UI 상태 관리
- [ ] 탭별 안내 메시지 표시

### TASK-104: 파일 업로드 영역 구현
- [ ] Streamlit file_uploader 위젯 추가
- [ ] 파일 형식 필터 설정 (.jmx, .c)
- [ ] Drag & Drop 영역 스타일링
- [ ] 업로드된 파일명 표시
- [ ] 파일 크기 표시 (MB 단위)
...
- [ ] 원본 코드 표시 영역 (읽기 전용)
- [ ] 변환된 코드 표시 영역 (읽기 전용)
- [ ] 구문 하이라이팅 적용 (st.code 활용)
- [ ] 라인 번호 표시
- [ ] 스크롤 동기화 (선택사항)

### TASK-106: 액션 버튼 구현
- [ ] Convert 버튼 생성 및 스타일링
- [ ] Download 버튼 생성 및 스타일링
- [ ] Clear 버튼 생성 및 스타일링
- [ ] 버튼 활성화/비활성화 로직
- [ ] 버튼 클릭 이벤트 핸들러 연결
